...
export default function KilnDetailScreen() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch kiln details");
      }
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    await queryClient.invalidateQueries({ queryKey: ["kiln", id] });
    setRefreshing(false);
  };

  const handleTrendPress = (parameter) => {
    Haptics.selectionAsync();
    router.push(`/(tabs)/kilns/trend/${parameter.id}`);
  };
    ...
    <View style={{ flex: 1, backgroundColor: "#F9FAFB" }}>
      ...
      >
        ...
        >
          ...
          <Flame size={28} color={statusColor} />
          <Text
            style={{
              fontSize: 24,
              fontWeight: "bold",
        ...
        >
          ...
          >
            Updated: {new Date(kiln.updated_at).toLocaleTimeString()}
          </Text>
        </View>
      </View>
      ...
      >
        ...
        {kiln.parameters && kiln.parameters.length > 0 ? (
          ...
          >
            ...
            >
              No PLC parameters are configured for this kiln
            </Text>
          </View>
        )}
      </ScrollView>
